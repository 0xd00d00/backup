...
class GmailAttachment:
    ...
    def __init__(self):
        ...
        # read ram, choi email
        self.email_list = parser.get('receiver','value1') + ',' + parser.get('receiver','value2') + ',' + parser.get('receiver', 'value3')
        # self.email_list = parser.get('receiver','value3') + ',' 
        # + parser.get('receiver','value2') + ',' + parser.get('receiver', 'value3')

        print("ram, choi !! ", self.email_list)
    ...
    def report(self, w_date, pass_list, fail_list):
        smtp = smtplib.SMTP('smtp.gmail.com', 587)
        smtp.starttls()
        ...
        print(w_date)
        msg['Subject'] = f"{w_date} " + " 기준, 인증자 리포트"
        smtp.sendmail('kkumhaha@gmail.com',self.email_list.split(','),msg.as_string())
