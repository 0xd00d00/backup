...
class GSpreadHelper:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.last_date_col1 = len(date_row_list)%27

            if (self.last_date_col%27 == 0):
                self.last_date_col1 = 1

            if (self.last_date_col/27 > 0):
                self.LEADING_CHAR = f"{chr(int(self.last_date_col/27) + 64)}"
    ...
    def UpdateForAuth(self, date, auth_list):
        # 날짜먼저 넣고
        col_tmp = f"{chr(self.last_date_col1 + 64)}"
        ...
        if int(self.last_date_col1)%27 == 0:
            self.last_date_col1 = 1

        if int(self.last_date_col1)/27 > 0:
            self.LEADING_CHAR = f"{chr(int(self.last_date_col1/27) + 64)}"
